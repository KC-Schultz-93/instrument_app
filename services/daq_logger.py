...
from instrument_app.services.daq_models import EventSummary, STFTResult, WaveformRecord
...
class DAQLogger:
    ...
    def save_raw(
        ...
        Saved arrays:
            voltage          â€” float64, volts
            time_ns          â€” float64, nanoseconds
            meta             â€” 1-element object array, JSON string with metadata
            stft_times_s     â€” float64, STFT window centers (seconds), if provided
            stft_freqs_hz    â€” float64, STFT frequency bins (Hz), if provided
            stft_power_db    â€” float64 2-D (FÃ—T), STFT magnitude (dBV), if provided
            stft_dominant_hz â€” float64, per-window dominant frequency, if provided
        """
