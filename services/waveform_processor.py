...
All methods are static â€” no state is held between traces.
...
from instrument_app.services.daq_models import STFTResult
...
class WaveformProcessor:
    ...
    def smooth(voltage: np.ndarray, window: int = 5) -> np.ndarray:
        ...
        voltage : ndarray
        window : int
            Smoothing window in samples. Forced to an odd integer â‰¥ 1.
        """
    ...
    def compute_fft(
        ...
        # Normalize: two-sided â†’ one-sided, scale by window sum for amplitude
    ...
    def compute_stft(
        ...
        evolution over time â€” useful for detecting mid-trace mass/charge changes.
        ...
        nperseg : int
            ...
            Must be â‰¤ len(voltage); clamped automatically.

        Returns
        -------
        STFTResult
            times_s          : shape (T,)    â€” window center times in seconds
            frequencies_hz   : shape (F,)    â€” frequency bins in Hz
            power_db         : shape (F, T)  â€” magnitude spectrum in dBV
            dominant_freq_track_hz : shape (T,) â€” dominant non-DC freq per window
        """
        n = len(voltage)
        if n < 4:
            empty_f = np.zeros(1)
