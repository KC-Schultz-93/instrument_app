...
All methods are instance methods but stateless relative to individual traces —
the only state held is the CDMSConfig calibration parameters for the run.

Charge model (CoolFET amplifier):
    Q [e] = V_peak [μV] / charge_cal_uv_per_e

m/z model (simple harmonic oscillator, to be refined with SIMION):
    m/z [Da/e] = trap_k_Da_Hz2 / f [Hz]²
    K_trap = 0 means uncalibrated — m/z and mass are returned as None.

Mass:
    mass [Da] = charge_e [e] × (m/z [Da/e])
"""
...
from Services.DAQModels import CDMSConfig, CDMSResult
...
class CDMSAnalyzer:
    ...
    def estimate_charge_e(self, peak_amplitude_v: float) -> float:
        ...
        Uses the CoolFET amplifier calibration:
            Q [e] = (amplitude [V] × 1e6) / charge_cal_uv_per_e
