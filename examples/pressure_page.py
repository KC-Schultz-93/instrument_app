...
class PressurePage(QWidget):
    ...
    def create_left_panel(self):
        ...
        # Pump controls — 2×2 grid
        pump_group = QGroupBox("Pumps")
        pump_grid = QGridLayout()
        self.tv850ia_group = QPumpControl("TV850i-A")
        self.tv850ia_group.runClicked.connect(lambda: self.send_command('S'))
        self.tv850ia_group.stopClicked.connect(lambda: self.send_command('X'))
        self.tv850ib_group = QPumpControl("TV850i-B")
        self.tv850ib_group.runClicked.connect(lambda: self.send_command('S'))
        self.tv850ib_group.stopClicked.connect(lambda: self.send_command('X'))
        self.tv350ia_group = QPumpControl("TV350i-A")
        self.tv350ia_group.runClicked.connect(lambda: self.send_command('S'))
        self.tv350ia_group.stopClicked.connect(lambda: self.send_command('X'))
        self.tv350ib_group = QPumpControl("TV350i-B")
        self.tv350ib_group.runClicked.connect(lambda: self.send_command('S'))
        self.tv350ib_group.stopClicked.connect(lambda: self.send_command('X'))
        pump_grid.addWidget(self.tv850ia_group, 0, 0)
        pump_grid.addWidget(self.tv850ib_group, 0, 1)
        pump_grid.addWidget(self.tv350ia_group, 1, 0)
        pump_grid.addWidget(self.tv350ib_group, 1, 1)
