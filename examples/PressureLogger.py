...
Logs UHV and Foreline pressure readings (A and B channels each) to CSV files
organized by date. Files rotate daily at 3:00 AM.

Directory structure:
    Recorded Data/Recorded Pressures/YYYY/MM/
        YYYY_MM_DD_UHV_Pressure.csv      (columns: timestamp, elapsed_minutes, uhv_torr_a, uhv_torr_b)
        YYYY_MM_DD_Foreline_Pressure.csv (columns: timestamp, elapsed_minutes, fore_torr_a, fore_torr_b)
"""
...
class PressureLogger:
    ...
    def _open_file(self, file_path, file_type):
        ...
        if is_new:
            if file_type == 'uhv':
                f.write("timestamp,elapsed_minutes,uhv_torr_a,uhv_torr_b\n")
            else:
                f.write("timestamp,elapsed_minutes,fore_torr_a,fore_torr_b\n")
            f.flush()
    ...
    def log_pressure(self, elapsed_minutes, uhv_torr_a, uhv_torr_b, fore_torr_a, fore_torr_b):
        """
        Log pressure readings to files.

        Args:
            elapsed_minutes: Elapsed time in minutes (from Arduino timestamp)
            uhv_torr_a: UHV-A pressure in Torr
            uhv_torr_b: UHV-B pressure in Torr
            fore_torr_a: Foreline-A pressure in Torr
            fore_torr_b: Foreline-B pressure in Torr
        """
        ...
        # Write UHV (always)
        if self._uhv_file:
            self._uhv_file.write(
                f"{timestamp},{elapsed_minutes:.4f},{uhv_torr_a:.6e},{uhv_torr_b:.6e}\n"
            )
        ...
        if current_time - self._last_foreline_log_time >= 10.0:
            if self._fore_file:
                self._fore_file.write(
                    f"{timestamp},{elapsed_minutes:.4f},{fore_torr_a:.6e},{fore_torr_b:.6e}\n"
                )
