...
def check_file(filename, description):
    ...
    print(f"âœ“ File exists: {filename} ({file_size} bytes)")

    with open(filename, 'r') as f:
        lines = f.readlines()

    print(f"âœ“ Contains {len(lines)} lines")
    print("\nFirst 10 lines:")
...
def main():
    ...
    if not has_serial:
        print("âŒ No serial connection attempted")
        print("   â†’ Try connecting to COM3 in the GUI")
    elif has_serial:
        print("âœ“ Serial connection was attempted")

    if not has_data:
        print("âŒ No data received from Arduino")
        print("   â†’ Check if Arduino is running and sending data")
        print("   â†’ Run: python Tests/test_serial.py")
    elif has_data:
        print("âœ“ Data is being received from Arduino")

    if not has_chart:
        print("âŒ Chart update never called")
        print("   â†’ This is unusual - the chart should initialize even without data")
    elif has_chart:
        print("âœ“ Chart update is being called")

        # Check if we have enough data
        with open('chart_update.log', 'r') as f:
            content = f.read()
            if 'Not enough data' in content:
                print("   âš ï¸  Still waiting for 2+ data points")
                print("   â†’ Wait a few seconds after connecting")
            if 'Final HTML generated' in content:
                print("   âœ“ Graph HTML is being generated")
                print("   â†’ Graph should be visible!")

    if has_crash:
        print("âŒ Program crashed at some point")
        print("   â†’ Check crash_log.txt for details")
