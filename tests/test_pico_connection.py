...
from instrument_app.services.daq_models import AcquisitionConfig
...
def test_timebase_resolution():
    ...
    print(f"  Requested: 200 ns  â†’  index={idx}, actual={actual_ns} ns")
    assert actual_ns > 0
...
def test_single_block_acquisition():
    ...
    config = AcquisitionConfig(
        ...
        voltage_range_v=2.0,
        coupling="DC",
        sample_interval_ns=200,   # 200 ns â‰ˆ 5 MS/s
        num_samples=5000,
    )
...
def test_multiple_blocks():
    ...
    config = AcquisitionConfig(
        ...
        voltage_range_v=2.0,
        coupling="DC",
        sample_interval_ns=1000,   # 1 Âµs = 1 MS/s
        num_samples=1000,
    )
