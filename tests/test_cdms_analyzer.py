...
from instrument_app.services.daq_models import CDMSConfig
...
def test_charge_known_sample():
    ...
    with the CoolFET amplifier (cal factor 0.64 ÂµV/e).
    Invert: Q = 5.76 mV / 0.64 ÂµV/e = 9000 e.
    """
...
def test_charge_custom_cal_factor():
    ...
    result = analyzer.estimate_charge_e(1e-3)  # 1 mV = 1000 ÂµV
...
def test_mz_formula():
    """m/z = K / fÂ² should hold for a known example.

    K = 1e13 DaÂ·HzÂ², f = 100 kHz â†’ m/z = 1e13 / (1e5)Â² = 1e13 / 1e10 = 1000 Da/e
    """
    K = 1e13       # DaÂ·HzÂ²
    f = 100_000.0  # 100 kHz
...
def test_mass_formula():
    """mass [Da] = charge_e [e] Ã— mz [Da/e]."""
    analyzer = CDMSAnalyzer(CDMSConfig())
    mass = analyzer.compute_mass_Da(9000.0, 1000.0)  # 9000e Ã— 1000 Da/e
...
def test_analyse_with_calibration():
    ...
    K = 1e13 DaÂ·HzÂ², f = 100 kHz â†’ m/z = 1000 Da/e; charge = 9000 e â†’ mass = 9 MDa
    """
    K = 1e13
    f = 100_000.0
